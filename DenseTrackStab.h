...
//String face_cascade_name = "C:/Users/christian/opencv/data/haarcascades/haarcascade_frontalface_alt.xml";
//CascadeClassifier face_cascade;

char* video;
bool show_track = true; // set show_track = 1, if you want to visualize the trajectories
...
int scale_num = 8;
const float scale_stride = float(sqrt(2));
char* bb_file = NULL;
...
const float min_flow = float(0.4);
...
const float min_var = float(sqrt(3));
const float max_var = 50;
const float max_dis = 20;

enum TrackColor {RED, GREEN, WHITE};
...
public:
	...
	BoundBox(float a1, float a2, float a3, float a4)
	{
		TopLeft.x = a1;
		TopLeft.y = a2;
		BottomRight.x = a3;
