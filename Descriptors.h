...
bool IsCameraMotion(std::vector<Point2f>& disp)
{
	...
	float disp_norm = float(1)/disp_sum;
	for (int i = 0; i < disp.size(); ++i)
		disp[i] *= disp_norm;
...
int CalculatePyrLevels(const Mat& frame)
{
	int rows = frame.rows, cols = frame.cols;
	int min_dim = std::min<int>(rows, cols);
	float min_size = float(min_dim);
...
void DrawTrack(const std::vector<Point2f>& point, const int index, const float scale, Mat& image, TrackColor color)
{
	int x;
	Scalar tColor;
	...
	for (int j = 1; j <= index; j++) {
		...
		switch(color)
		{
			case RED:   tColor = Scalar(0,0,x); break;
			case GREEN: tColor = Scalar(0,x,0); break;
			case WHITE: tColor = Scalar(x,x,x); break;
			default: break;
		}
		
		line(image, point0, point1, tColor, 2, 8, 0);
		point0 = point1;
	}
...
*/
		for(int i = 0; i < size/4; i++)
			cur_frame.BBs.push_back(BoundBox(a[i*5], a[i*5+1], a[i*5+2], a[i*5+3]));
...
void InitMaskWithBox(Mat& mask, std::vector<BoundBox>& bbs)
{
	...
	for(int k = 0; k < bbs.size(); k++) {
		BoundBox& bb = bbs[k];
		//printf("tlx: %f tly: %f brx: %f bry: %f\n", bb.TopLeft.x, bb.TopLeft.y, bb.BottomRight.x, bb.BottomRight.y);
...
void detectAndDisplay(Mat& image, const Mat& frame_gray, Frame& in_frame)
{
	...
	{
		float tlx = float(faces[i].x);
		float tly = float(faces[i].y);
		float brx = float(faces[i].x + faces[i].width);
		float bry = float(faces[i].y + faces[i].height);
		
		BoundBox faceBox = BoundBox(tlx, tly, brx, bry);
		in_frame.BBs.push_back(faceBox);
		
		if(show_track == 1)
		{
			rectangle(image, faces[i], Scalar(0,255,255), 2);
		}
	}
}
